...
public class AutoLoginUI : MonoBehaviour
{
    ...
    IEnumerator AutoLoginProcess(string username)
    {
        ...
        yield return StartCoroutine(UpdateProgress(0.08f, 0.15f, 0.3f));

        subText.text = "正在验证账号信息...";
        yield return StartCoroutine(UpdateProgress(0.15f, 0.28f, 0.8f));
        yield return StartCoroutine(PauseProgress(0.28f, 0.2f));
        yield return StartCoroutine(UpdateProgress(0.28f, 0.35f, 0.4f));
        ...
        subText.text = "正在预加载标题Logo...";
        Resources.Load<Sprite>("UI/Login/title_logo");
        yield return StartCoroutine(UpdateProgress(0.48f, 0.55f, 0.5f));
        yield return StartCoroutine(PauseProgress(0.55f, 0.1f));

        subText.text = "正在加载头像资源...";
        Resources.Load<Sprite>("UI/AvatarFrame");
        yield return StartCoroutine(UpdateProgress(0.55f, 0.62f, 0.4f));
        Resources.Load<Sprite>("UI/DefaultAvatar");
        yield return StartCoroutine(UpdateProgress(0.62f, 0.68f, 0.4f));
        yield return StartCoroutine(PauseProgress(0.68f, 0.15f));

        subText.text = "正在加载UI样式...";
        Resources.Load<UnityEngine.UIElements.PanelSettings>("UI/TitleScreenPanelSettings");
        yield return StartCoroutine(UpdateProgress(0.68f, 0.74f, 0.5f));
        Resources.Load<TextAsset>("UI/TitleScreen");
        yield return StartCoroutine(UpdateProgress(0.74f, 0.78f, 0.3f));
        Resources.Load<TextAsset>("UI/GlobalStyle");
        yield return StartCoroutine(UpdateProgress(0.78f, 0.82f, 0.3f));
        yield return StartCoroutine(PauseProgress(0.82f, 0.15f));

        subText.text = "正在加载字体...";
        Resources.Load<Font>("Fonts/zpix");
        yield return StartCoroutine(UpdateProgress(0.82f, 0.88f, 0.5f));

        subText.text = "正在初始化视频背景...";
        Resources.Load<Material>("Materials/VideoBackground");
        yield return StartCoroutine(UpdateProgress(0.88f, 0.92f, 0.4f));
        yield return StartCoroutine(PauseProgress(0.92f, 0.2f));

        subText.text = "正在初始化系统...";
        yield return StartCoroutine(UpdateProgress(0.92f, 0.95f, 0.3f));
        yield return StartCoroutine(PauseProgress(0.95f, 0.3f));
