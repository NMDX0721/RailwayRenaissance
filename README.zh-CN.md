...
整个项目结构为 AI 工具链优化——文档、代码、数据格式统一，用生成式 AI 就能实现自己的想法。
...
研发不是填进度条，而是选择下一个要解决的问题，并承受那些你没有解决的问题带来的后果。

### 区域解锁与政治系统

世界分为六个区域，每个区域包含多个城市。解锁一个区域需要满足信任阈值、支付连接费用、并完成一个剧情事件。每个区域有其主导政治倾向——**威权**、**市场**或**福利**——决定了当地政府如何评估玩家的表现。

政治倾向会随玩家行为而偏移。威权地区奖励稳定、惩罚动荡；市场地区奖励盈利、惩罚低效；福利地区奖励服务、惩罚怠慢。玩家必须针对每个区域的政治气候调整策略，否则将面临补贴削减、检查加严，甚至线路被征收。

### RDA——**李式调度算法**（Ri Dispatch Algorithm）/ **铁路决策助手**（Railway Decision Assistant）

玩家的爷爷开发了**李式调度算法**（RDA，Ri Dispatch Algorithm），一种基于实时客流数据优化列车路径的动态调度协议。玩家继承了该算法，以及一部运行其改良版的手机助手——**铁路决策助手**（RDA，Railway Decision Assistant），缩写同为 RDA。

**RDA** 系统随玩家线网扩张渐进解锁：初期提供基础排班建议，后期解锁预测性维护预警、自动排班和基于需求预测的动态票价调整。手机助手通过玩家的 API Key 接入金日成综合大学服务器，直接调用实验室中爷爷留下的原始调度算法。

### 施工队与燃料管理

扩张线网不只需要买新车。玩家需要派遣**施工队**铺设新轨道、修复损坏区段、升级车站。每支施工队有自己的技能成长和设备需求。
