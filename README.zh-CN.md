...
**智能调度系统实验室**是该校的旗舰研究设施，致力于下一代铁路调度算法的研发。其计算能力不亚于半岛上任何民用研究机构。实验室的主要研究方向——利用实时客流数据进行动态路径优化——代表了一个该大学已连续三个评估周期保持全球领先地位的领域。

现任研究员林彪悍是荣誉研究生，其基于Dijkstra自适应调度的研究成果已被国际运输经济研究所引用。他使用的终端是一台千里马系列工作站，连接至大学内部仿真网格——该网络最初为朝鲜国家铁路调度系统开发，后在2072年大规模线路关闭后转为学术研究用途。

朝南的窗户可以看到大同江和主体思想塔。办公室采用标准机构风格配置：硬木书桌、存放技术手册和国家出版的交通史的书架，以及展示大学创始宪章和定期安全表彰的墙面。

实验室实行24小时开放制度。在书桌上，终端旁边，放着一本翻旧的《铁路工程手册》。扉页上有一行褪色的手写字迹：
